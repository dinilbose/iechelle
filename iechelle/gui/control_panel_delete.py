...
class Control_panel(Environment):
        ...
        def __init__(self):
            ...
            self.diff_id_mycatalog=utils_delete.difference(self.id_mycatalog)
            ...
            self.id_temp=utils_delete.difference(self.id_mycatalog)

            self.catalog_all=mycatalog_delete.pointer(catalog='mycatalog')
            self.prgm_ds9=None
            ...
            if self.env.Samp_status.active[0]==0:
                if self.env.Samp_selection.value=='Display':
                    if self.display==None:
                        ...
                        self.receiver_variable=utils_delete.Receiver(self.display)
        ...
        def message_recieve(self):
            r=utils_delete.Receiver(self.display)
            ...
            new=utils_delete.difference(self.id_mycatalog)
        ...
        def ds9_command(self):
            ...
            w=mycatalog_delete.pointer(catalog='mycatalog',id_mycatalog=id_mycatalog)
            ...
            filename=mycatalog_delete.filename(name='cluster_ffi',id_mycatalog=id_mycatalog,sector=self.env.sector)[0]
            ...
            if self.env.ds9_catalog_status.active[0]==0:
                ...
                name=mycatalog_delete.filename(name='temp',sector=self.env.sector).joinpath('whole_gaia.csv')
        ...
        def setup_whole_gaia(self):
            ...
            self.env.catalog_whole_gaia=pd.read_csv(mycatalog_delete.filename(name='whole_gaia_data',id_mycatalog=id_mycatalog,sector=self.env.sector))
